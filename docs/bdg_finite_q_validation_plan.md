...
core finite-q 计算位于 `src/lno327/finite_q_engine.py`。可复用的基础诊断 helper 保留在 `validation/lib/finite_q_diagnostics.py`。项目级 validation workflow 位于 `validation/scripts/bdg_finite_q/`，统一轻量 report 与复现命令位于 `validation/outputs/finite_q_ward/`。

## 验证顺序

1. **q=0 响应定义对齐**  
   使用统一入口 `validation/scripts/bdg_finite_q/q0_bdg_response_alignment.py --pairings normal onsite_s spm dwave --explain`。当前状态为 `spm: convention_aware_pass`、`dwave: intraband_aware_pass`、`normal/onsite_s: diagnostic_only_not_passed`。`dwave` 旧 raw-vs-total mismatch 保持可见，但由 local intraband / `-f'(E)` 贡献解释；finite-q raw bubble 在直接 `q=0` 对齐 local interband paramagnetic kernel。q=0 对齐是前置定义检查，不是 Ward closure proof。
...
- `dwave` q=0 raw-vs-total mismatch 已由 intraband-aware 诊断解释，不再作为未解释 raw-bubble/vertex mismatch。
- finite-q Ward closure 仍未完成，q=0 pass 不代表 finite-q Ward closed。
...
- `validation/scripts/bdg_finite_q/` 下的 workflow 是 validation 层，不是稳定 public API；raw arrays、大型 CSV、expanded logs 和 cache tensors 不应提交到 `validation/outputs/finite_q_ward/`。
