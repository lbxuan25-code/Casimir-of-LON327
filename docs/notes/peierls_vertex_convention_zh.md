...
本文档记录 Peierls current vertex 与 contact vertex 的顶角级推导和符号约定。它不是最终 conductivity、reflection 或 Casimir 结论，也不决定 response-level physical current sign。
...
Peierls substitution 包括一阶 current vertex $\Gamma_i^H$ 和二阶 contact vertex $\Lambda_{ij}^H$。contact term 只是 Peierls 展开的一部分，不等于整个 Peierls 方法。当前顶角级结果支持 $\Gamma_i^H$ 和 $\Lambda_{ij}^H$ 本身基本可信；response-level 闭合还需要单独固定 physical current sign、contact response sign、Ward $q$-sign 和 Kubo convention。
