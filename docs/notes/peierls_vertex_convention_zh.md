...
vertex-level：确认 Peierls 展开给出的 Hamiltonian vector vertex $\Gamma_i^H$ 和
Hamiltonian contact vertex $\Lambda_{ij}^H$。
...
$\Gamma_i^H=\delta H/\delta A_i$ 的正确顶角级符号。
...
因此 physical current vertex 是 $\Gamma_i^{\mathrm{phys}}=-\Gamma_i^H$。
不要把“顶角级 plus sign 通过”误读成“physical-current response 必须使用
$\Gamma_i^H$ 的 plus sign”。
...
Stage 4.2A 已验证该 Hamiltonian contact vertex $\Lambda_{ij}^H$ 的 $q=0$ mass
limit、Hermiticity 和 $\Lambda_{xy}=\Lambda_{yx}$。
...
$K_{ij}^{\mathrm{phys}}=-\langle\Lambda_{ij}^H\rangle$，其最终使用不由本文档单独决定，而由
...
Peierls substitution 包括一阶 Hamiltonian vector vertex $\Gamma_i^H$ 和二阶
Hamiltonian contact vertex $\Lambda_{ij}^H$。$\Lambda_{ij}^H$ 只是 Peierls 展开的一部分，
