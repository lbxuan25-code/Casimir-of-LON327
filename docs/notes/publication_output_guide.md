...
- `validation/outputs/archive/response/local_sheet_imag/`：展示 normal / $s_{\pm}$ / $d$-wave 的 local
  response 对比和 C4 symmetry diagnostics。

**只作为方法边界或附录说明**

- `validation/outputs/archive/response/static_policy_comparison/`：说明 $n=0$ policy 的保守处理。
- `validation/outputs/archive/response/unit_audit/`：说明单位转换约定。
- `validation/outputs/archive/response/nonlocal_interface/`：说明 finite-$q_{\parallel}$ 尚未实现。

**不作为论文物理结论**

- `validation/outputs/archive/smoke/smoke/`：工程链路冒烟测试。
- 当前 `validation/outputs/casimir/`：未来正式 Casimir 计算预留目录。
...
python validation/scripts/response/compare_local_sheet_response_imag.py \
  ...
  --eta 0.0001 \
  --output-prefix validation/outputs/archive/response/local_sheet_imag/data/local_sheet_response_imag
```
