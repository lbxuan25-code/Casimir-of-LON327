...
- `outputs/response/local_sheet_imag/`：展示 normal / $s_{\pm}$ / $d$-wave 的 local
  response 对比和 C4 symmetry diagnostics。

**只作为方法边界或附录说明**

- `outputs/response/static_policy_comparison/`：说明 $n=0$ policy 的保守处理。
- `outputs/response/unit_audit/`：说明单位转换约定。
- `outputs/response/nonlocal_interface/`：说明 finite-$q_{\parallel}$ 尚未实现。

**不作为论文物理结论**

- `outputs/smoke/`：工程链路冒烟测试。
...
python scripts/compare_local_sheet_response_imag.py \
  ...
  --eta 0.0001 \
  --output-prefix outputs/response/local_sheet_imag/data/local_sheet_response_imag
```
