...
4. **Future Casimir Torque**
   ...
   - 当前已显式补齐两个 Casimir 前置边界接口：中性 sheet-conductivity convention audit、
     $n=0$ Matsubara static policy。
   ...
   - 正式 Casimir 阶段仍需选择具体物理方案：$n=0$ 物理处理，
     或外场/应变/表面取向等各向异性来源。
...
python scripts/pairing/inspect_gap_structure.py --kind dwave --delta0 0.04 --nk 80 --energy-window 0.05 --node-tolerance 0.001
python scripts/bdg/compute_bdg_paramagnetic_kernel_imag.py --kind spm --delta0 0.04 --nk 24 --temperature 30 --matsubara-index 1
python scripts/bdg/diagnose_bdg_diamagnetic_kernel.py --kinds spm dwave --delta0 0.04 --nk 24 --temperature 30
python scripts/bdg/diagnose_bdg_total_kernel_imag.py --kinds spm dwave --delta0 0.04 --nk 24 --temperature 30 --matsubara-min 1 --matsubara-max 8
python scripts/bdg/diagnose_superconducting_response_imag.py --kinds spm dwave --delta0 0.04 --nk 24 --temperature 30 --matsubara-min 1 --matsubara-max 8
...
- `SheetConductivityConvention` 和 `StaticResponsePolicy` 已把这些边界变成显式接口状态，
  但还没有把当前 response 升级为正式 Casimir input。
