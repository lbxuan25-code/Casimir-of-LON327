...
| `normal_physical_density_current_response_imag_axis` | physical-current response candidate | $\Pi_{00}=B_{00}$，$\Pi_{0j}=B_{0j}$，$\Pi_{i0}=-B_{i0}$，$\Pi_{ij}=-B_{ij}-C_{ij}^{\mathrm{code}}$ | 显式使用 observable $J=(\rho,-V_x,-V_y)$、source $P=(\rho,V_x,V_y)$ 和 $K_{ij}^{\mathrm{phys}}=-\langle M_{ij}\rangle$ 的 readable candidate | 否；source/observable mismatch 已在 Stage 4.8 修复；equal-time completion 仍待审计 |
...
source/observable vertex split; equal-time / commutator completion remains
UNRESOLVED.
...
Stage 4.7 曾把 spatial bubble shorthand 写成 source-source 形式。Stage 4.8
修正了这个 shorthand：response 定义
$\delta\langle J_\mu\rangle/\delta a_\nu$ 要求 bubble 使用 observable/source
两套顶角，主实现现在为
