...
- 冻结的 `todo4_representative_v1` 资格矩阵：5 个 geometry plan、16 个 geometry
  point、20 个 exact response identity、10 个独立 populate group；
- validation-only 的 `plan → preflight → populate → geometry → legacy → verify`
  分阶段入口，population 与 strict read-only geometry 明确分离；
- 原子 qualification artifacts、source commit/manifest/plan SHA，以及 deterministic
  shard partition；
- legacy replay 前对每个两板点的 working N 与 exact primary shift 做独立兼容性预检，
  不允许静默 common-N/common-shift 搜索；
- repository-level import guards，保持 material、persistence、cache request、geometry
  planning、geometry execution、qualification campaign/IO 与 observable 层的隔离；
- 所有新对象保持 diagnostic-only。

TODO 4 尚未完成的 qualification 项：

- 运行已冻结的 SPM 与 d-wave 真实 microscopic representative points；
- 生成 exact n=0 与 n=1、轴向/斜向 q、零角/非零相对角的真实 old/new 记录；
- 确认所有两板点具有匹配的 working N 与 primary shift，或将不匹配明确标为
  qualification unresolved；
- 使用真实 old/new 点值生成小型 fixed-outer-Q replay 记录；
- 审查本地运行 artifacts 后再决定 PR 是否 ready/merge。
...
资格运行中只有显式 `populate` 阶段允许调用 response ladder 并写 certified cache。
后续 `geometry`、`legacy` 的新路线侧和 `verify` 均重新以 read-only 方式加载。
...
```text
本地运行冻结的 TODO 4 representative qualification
→ 审查 cache/compatibility/geometry/legacy/fixed-outer artifacts
→ TODO 4 review/merge
