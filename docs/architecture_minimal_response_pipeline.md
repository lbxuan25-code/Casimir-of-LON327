...
the gauge-aware finite-q form factor. Diagnostic alternatives such as midpoint
or symmetric `k±q/2` phase vertices remain explicit options.
...
The generic engine lives in `lno327.finite_q_engine`. It accepts a pairing
ansatz and computes:
...
generic calculation to the engine.
