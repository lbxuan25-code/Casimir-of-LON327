...
def _complex_norm_payload(payload: dict[str, object]) -> str:
    return str(payload)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"shifted_average: {payload.get('shifted_mesh_average', {}).get('average_order')}")
    for row in payload.get("results", []):
        diag = row.get("diagnostics", {})
        print(
            "q={q} physical_norm={phys} gauge_row={grow} gauge_col={gcol} cond={cond}".format(
                q=row.get("q_model"),
