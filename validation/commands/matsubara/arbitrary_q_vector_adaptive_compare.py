...
This command is diagnostic only.  It never authorizes Casimir input or replaces
formal fixed-periodic-grid qualification.
"""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    fixed_grid = build_periodic_bz_grid(args.fixed_N, (0.5, 0.5))
    fixed_cache = build_material_grid_cache(
        spec=model.spec,
        ansatz=ansatz,
        pairing=pairing,
    ...
    fixed = integrate_arbitrary_q_periodic_bz(
        spec=model.spec,
        ansatz=ansatz,
        pairing=pairing,
        xi_eV_values=xi,
    ...
    adaptive_cache = build_reusable_hierarchical_material_node_cache(
        spec=model.spec,
        ansatz=ansatz,
        pairing=pairing,
        temperature_K=args.temperature_K,
    ...
    adaptive = integrate_arbitrary_q_vector_adaptive_cached(
        spec=model.spec,
        ansatz=ansatz,
        pairing=pairing,
        xi_eV_values=xi,
    ...
    fixed_states = _physical(fixed, q, args)
    adaptive_states = _physical(adaptive, q, args)
    frequency_rows = []
        ...
        frequency_rows.append(
            {
                "n": int(n_value),
                "primary": _mixed(
                    fixed_states[index]["primary"],
                    adaptive_states[index]["primary"],
                    atol=args.comparison_atol,
                    rtol=args.comparison_rtol,
                ),
                "reflection": _mixed(
                    fixed_states[index]["reflection"],
                    adaptive_states[index]["reflection"],
                    atol=args.comparison_atol,
                    rtol=args.comparison_rtol,
                ),
                "logdet": _mixed(
                    fixed_states[index]["logdet"],
                    adaptive_states[index]["logdet"],
                    atol=args.comparison_atol,
                    rtol=args.comparison_rtol,
                ),
    ...
    primitive = _mixed(
        fixed.packed_primitives,
        adaptive.packed_primitives,
        atol=args.comparison_atol,
        rtol=args.comparison_rtol,
    )
    passed = bool(
        ...
        and all(
            row["primary"]["passed"]
            and row["reflection"]["passed"]
            and row["logdet"]["passed"]
            and row["fixed_physical"]["passed"]
            and row["adaptive_physical"]["passed"]
            for row in frequency_rows
        )
    )
    payload = {
        "schema": "arbitrary-q-vector-adaptive-compare-v1",
