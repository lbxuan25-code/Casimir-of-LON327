"""Global vector-valued iterated-adaptive exact-static d-wave validation.

The runner integrates the full periodic Brillouin zone with nested ``quad_vec``
quadrature.  Every primitive electromagnetic/collective block, the Goldstone
counterterm, and the Ward RHS share the same adaptive nodes and weights.  One
amplitude/phase Schur complement is applied only after the full BZ integral.

Both nesting orders should be run.  Agreement between ``xy`` and ``yx`` is an
independent ridge-resolution diagnostic, but this prototype remains fail-closed and
never promotes one run directly to a production reference.
"""
...
DEFAULT_OUTPUT = Path(
    ...
    "dwave_q003_002_T10_iterated_adaptive.csv"
)
...
def _summary_text(
    ...
    lines = [
        "d-wave global iterated-adaptive exact-static scan",
        "=" * 57,
        ...
        "Per-order results",
        "-----------------",
        " order  points   quad_err  success      chi_bar       Dbar_T      "
    ...
    if comparison.get("available", False):
        lines.extend(
            [
                ...
                f"relative_raw_longitudinal_xy_yx = "
                f"{float(comparison['relative_raw_longitudinal']):.3e}",
    ...
    lines.extend(
        [
            ...
            "",
            "Order agreement is only a feasibility diagnostic. A production reference "
            "still requires tolerance tightening and comparison against an independent "
            "high-budget complete-periodic calculation.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    for order in args.orders:
        print(f"starting global iterated-adaptive order={order}", flush=True)
        result = integrate_dwave_static_order(
            context,
            order=order,
            options=adaptive_options,
        )
        processed = postprocess_merged(result.components, result.rhs, physical)
    ...
    if comparison["available"]:
        ...
        relative_raw = relative_difference(
            yx["raw_longitudinal"], xy["raw_longitudinal"]
        )
        physical_disagreement = max(relative_chi, relative_dbar)
        order_pass = bool(physical_disagreement <= args.order_agreement_tolerance)
        comparison.update(
            {
                "relative_chi": relative_chi,
                "relative_dbar": relative_dbar,
                "relative_raw_longitudinal": relative_raw,
                "physical_order_disagreement": physical_disagreement,
                "order_agreement_pass": order_pass,
            }
        )
    else:
        comparison.update(
            {
                ...
                "relative_raw_longitudinal": float("nan"),
    ...
    comparison["adaptive_feasibility_pass"] = bool(
        ...
        and all(bool(row["ward_passed"]) for row in rows)
    ...
    payload = {
        "schema": "dwave_global_iterated_adaptive_v1",
