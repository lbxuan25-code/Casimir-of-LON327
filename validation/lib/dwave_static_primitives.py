"""Pointwise primitive response densities for the exact-static d-wave path.

The evaluator returns every quantity needed before the nonlinear collective Schur
step: the unified electromagnetic/collective bubble, electromagnetic contact term,
Goldstone counterterm, phase direct term, and all three analytic Ward-RHS pieces.
A numerical BZ integrator must integrate this complete vector on shared nodes and
weights before calling :func:`assemble_dwave_static_primitives`.
"""
...
class DWaveStaticIntegrandContext:
    ...
    def __post_init__(self) -> None:
        ...
        if float(self.config.omega_eV) != 0.0:
            raise ValueError("d-wave static primitive evaluation requires omega_eV == 0")
        if str(self.options.current_vertex) != "peierls":
            raise ValueError("d-wave static response requires current_vertex='peierls'")
        if str(self.options.collective_mode) != "amplitude_phase":
            raise ValueError(
                "d-wave static response requires collective_mode='amplitude_phase'"
            )
        if str(self.options.collective_counterterm) != "goldstone_gap_equation":
            raise ValueError(
                "d-wave static response requires the Goldstone gap-equation counterterm"
            )
    ...
    def evaluate_complex(self, k_points: np.ndarray) -> np.ndarray:
        ...
        for index, (kx_value, ky_value) in enumerate(points):
            ...
            if len(collective_vertices) != _COLLECTIVE_CHANNELS:
                raise ValueError("d-wave primitive evaluator requires two collective channels")
            ...
            if len(collective_zero) != _COLLECTIVE_CHANNELS:
                raise ValueError("d-wave counterterm requires two collective channels")
            eta2_band = (
    ...
    def evaluate_real(self, kx: float, ky: float) -> np.ndarray:
        """Scalar callback for the vector-valued BZ quadrature."""
...
def build_dwave_static_integrand_context(
    ...
    """Build the canonical exact-static d-wave pointwise primitive evaluator."""
...
def assemble_dwave_static_primitives(
    ...
    *,
    ...
    """Assemble one response only after the complete primitive BZ integral."""

    vector = np.asarray(primitive_vector, dtype=complex).reshape(-1)
    if (
        vector.shape != (_COMPLEX_WIDTH,)
        or not np.isfinite(vector.real).all()
        or not np.isfinite(vector.imag).all()
    ):
        raise ValueError(
            f"primitive_vector must be a finite complex vector of width {_COMPLEX_WIDTH}"
        )
    ...
    component_metadata.update(
        {
            ...
            "all_primitive_channels_share_quadrature_nodes": True,
            **dict(metadata or {}),
        }
    )
    ...
    rhs = PrimitiveWardRHS(
        ...
        metadata={
            ...
            "source": "shared-node d-wave primitive BZ integral",
            "equal_forward": ward_equal.copy(),
            "delta_v_mid": ward_delta_v.copy(),
            "qM_mid": ward_contact.copy(),
            "all_terms_share_quadrature_nodes": True,
            **dict(metadata or {}),
        },
    )
    primitive_metadata = {
