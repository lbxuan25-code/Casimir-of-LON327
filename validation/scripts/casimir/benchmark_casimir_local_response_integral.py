#!/usr/bin/env python3
"""Benchmark local-response Casimir Matsubara/k/phi integration.
...
def benchmark_casimir_local_response_integral(
    kinds: list[str],
...
def output_paths(output_prefix: Path) -> tuple[Path, Path, Path, Path, Path, Path, Path]:
    ...
    project_data_root = (ROOT / "validation" / "outputs" / "archive" / "casimir" / "local_response_integral" / "data").resolve()
    if resolved_prefix.is_relative_to(project_data_root):
        figure_dir = ROOT / "validation" / "outputs" / "archive" / "casimir" / "local_response_integral" / "figures"
    else:
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-prefix",
        type=Path,
        default=ROOT / "validation" / "outputs" / "archive" / "casimir" / "local_response_integral" / "data" / "local_response_integral",
    )
    args = parser.parse_args()
    data = benchmark_casimir_local_response_integral(
        args.kinds,
        args.distance_list,
        args.theta_list,
        args.matsubara_min,
