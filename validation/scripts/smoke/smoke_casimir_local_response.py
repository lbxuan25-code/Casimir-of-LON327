...
def output_paths(output_prefix: Path) -> tuple[Path, Path]:
    ...
    project_data_root = (ROOT / "validation" / "outputs" / "archive" / "smoke" / "smoke" / "casimir_local_response" / "data").resolve()
    if resolved_prefix.is_relative_to(project_data_root):
        figure_dir = ROOT / "validation" / "outputs" / "archive" / "smoke" / "smoke" / "casimir_local_response" / "figures"
    else:
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-prefix",
        type=Path,
        default=ROOT / "validation" / "outputs" / "archive" / "smoke" / "smoke" / "casimir_local_response" / "data" / "casimir_local_response_smoke",
    )
