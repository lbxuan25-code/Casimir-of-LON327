"""Helpers for backwards-compatible validation script entry points."""
...
def load_impl(wrapper_file: str, relative_impl: str) -> ModuleType:
    ...
    module_name = f"_lno327_validation_script_impl_{impl_path.parent.name}_{impl_path.stem}"
    spec = importlib.util.spec_from_file_location(module_name, impl_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load validation script implementation: {impl_path}")
