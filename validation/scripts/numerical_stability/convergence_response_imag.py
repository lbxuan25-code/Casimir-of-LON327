#!/usr/bin/env python3
"""Benchmark imaginary-axis response convergence in nk, eta, and Matsubara index.

This is a response-layer diagnostic only. It checks whether normal/spm/dwave
local imaginary-axis response data are stable against numerical choices before
any future Casimir integration is attempted.
"""
...
DEFAULT_NK_LIST = (8, 12, 16, 24, 32)
DEFAULT_ETA_LIST = (1e-3, 5e-4, 1e-4)
DEFAULT_MATSUBARA_LIST = (1, 2, 5, 10)
...
REQUIRED_NPZ_FIELDS = {
    ...
    "relative_change_vs_smallest_eta",
    "convergence_status",
    "diagnosis",
    "notes",
}
...
def _relative_change(value: complex, reference: complex) -> float:
    if abs(reference) < RATIO_EPS and abs(value) < RATIO_EPS:
        return 0.0
...
def _evaluate_response(
    ...
    return (
        ...
        response.matrix,
        sheet.tensor.xx,
        reflection.tensor.xx,
        diagnostics,
    )


def convergence_response_imag(
    kinds: list[str],
    nk_list: list[int],
    eta_list: list[float],
    ...
    """Run the imaginary-axis response convergence benchmark."""

    if not kinds:
        raise ValueError("kinds must be non-empty")
    if any(kind not in KINDS for kind in kinds):
        raise ValueError("unknown kind")
    if not nk_list or any(nk <= 0 for nk in nk_list):
        raise ValueError("nk_list must contain positive integers")
    ...
    nk_values = np.asarray(nk_list, dtype=int)
    eta_values = np.asarray(eta_list, dtype=float)
    matsubara_values = np.asarray(matsubara_list, dtype=int)
    rows = [
    ...
    data: dict[str, np.ndarray] = {
        ...
        "relative_change_vs_smallest_eta": np.full(row_count, np.nan, dtype=float),
        "convergence_status": np.empty(row_count, dtype="U128"),
    ...
    smallest_eta = float(np.min(eta_values))
    for index, (kind, nk, eta, n) in enumerate(rows):
        nk_ref_index = index_by_key.get((kind, largest_nk, eta, n))
        eta_ref_index = index_by_key.get((kind, nk, smallest_eta, n))
        if nk_ref_index is not None:
            data["relative_change_vs_largest_nk"][index] = _relative_change(
                data["response_xx"][index],
                data["response_xx"][nk_ref_index],
            )
        if eta_ref_index is not None:
            data["relative_change_vs_smallest_eta"][index] = _relative_change(
                data["response_xx"][index],
                data["response_xx"][eta_ref_index],
            )
    ...
    for index in range(row_count):
        ...
        diagnosis_warnings: list[str] = []
        if data["relative_change_vs_largest_nk"][index] > CONVERGENCE_TOLERANCE:
            convergence_warnings.append("warning_nk_not_converged")
        if data["relative_change_vs_smallest_eta"][index] > CONVERGENCE_TOLERANCE:
            convergence_warnings.append("warning_eta_sensitive")
        if (
            ...
            diagnosis_warnings.append("warning_symmetry")
        if not np.isfinite(data["response_xx"][index]):
            diagnosis_warnings.append("warning_nonfinite_response")
        if not np.isfinite(data["sheet_conductivity_xx"][index]):
            diagnosis_warnings.append("warning_nonfinite_sheet_conductivity")
        data["convergence_status"][index] = _join_status(convergence_warnings)
        data["diagnosis"][index] = _join_status(diagnosis_warnings)
...
def output_paths(output_prefix: Path) -> tuple[Path, Path, Path, Path, Path, Path, Path]:
    ...
    project_data_root = (ROOT / "validation" / "outputs" / "archive" / "response" / "convergence_imag" / "data").resolve()
    if resolved_prefix.is_relative_to(project_data_root):
        figure_dir = ROOT / "validation" / "outputs" / "archive" / "response" / "convergence_imag" / "figures"
    else:
    ...
    return (
        ...
        figure_dir / f"{output_prefix.name}_response_xx_vs_eta.png",
        figure_dir / f"{output_prefix.name}_spm_dwave_diff_vs_nk.png",
        figure_dir / f"{output_prefix.name}_symmetry_vs_nk.png",
        figure_dir / f"{output_prefix.name}_reflection_vs_matsubara.png",
    )
...
def save_outputs(data: dict[str, np.ndarray], output_prefix: Path) -> tuple[Path, Path, Path, Path, Path, Path, Path]:
    ...
    npz_path, csv_path, nk_plot, eta_plot, diff_plot, symmetry_plot, matsubara_plot = paths
    npz_path.parent.mkdir(parents=True, exist_ok=True)
    nk_plot.parent.mkdir(parents=True, exist_ok=True)
    ...
    nk_values = list(dict.fromkeys(int(nk) for nk in data["nk"]))
    reference_eta = min(eta_values)
    reference_nk = max(nk_values)

    fig_nk, ax_nk = plt.subplots(figsize=(7.0, 4.2), constrained_layout=True)
    for kind in kinds:
        for n in n_values:
            mask = (data["kind"] == kind) & np.isclose(data["eta_eV"], reference_eta) & (data["matsubara_index"] == n)
            ax_nk.plot(data["nk"][mask], data["response_xx"][mask].real, marker="o", label=f"{kind} n={n}")
    ax_nk.set_xlabel(r"$N_k$")
    ax_nk.set_ylabel(r"Re response$_{xx}$")
    ax_nk.set_title(rf"response convergence in $N_k$ at $\eta={reference_eta:g}$")
    style_publication_axis(ax_nk)
    save_publication_figure(fig_nk, nk_plot)
    plt.close(fig_nk)

    fig_eta, ax_eta = plt.subplots(figsize=(7.0, 4.2), constrained_layout=True)
    for kind in kinds:
        for n in n_values:
            mask = (data["kind"] == kind) & (data["nk"] == reference_nk) & (data["matsubara_index"] == n)
            order = np.argsort(data["eta_eV"][mask])
            ax_eta.plot(data["eta_eV"][mask][order], data["response_xx"][mask].real[order], marker="o", label=f"{kind} n={n}")
    ax_eta.set_xscale("log")
    ax_eta.set_xlabel(r"$\eta$ (eV)")
    ax_eta.set_ylabel(r"Re response$_{xx}$")
    ax_eta.set_title(rf"response eta sensitivity at $N_k={reference_nk}$")
    ...
    for n in n_values:
        mask = (data["kind"] == "spm") & np.isclose(data["eta_eV"], reference_eta) & (data["matsubara_index"] == n)
        if np.any(mask):
            ax_diff.plot(data["nk"][mask], data["spm_dwave_rel_diff_xx"][mask], marker="o", label=f"n={n}")
    ax_diff.set_xlabel(r"$N_k$")
    ax_diff.set_ylabel(r"relative $s_{\pm}$-$d$ diff")
    ax_diff.set_title(rf"$s_\pm$-$d$ response difference at $\eta={reference_eta:g}$")
    ...
    for kind in kinds:
        mask = (data["kind"] == kind) & np.isclose(data["eta_eV"], reference_eta) & (data["matsubara_index"] == n_values[0])
    ...
    ax_sym.set_title(rf"C4 diagnostics at $\eta={reference_eta:g}, n={n_values[0]}$")
    style_publication_axis(ax_sym)
    save_publication_figure(fig_sym, symmetry_plot)
    plt.close(fig_sym)

    fig_matsu, ax_matsu = plt.subplots(figsize=(7.0, 4.2), constrained_layout=True)
    for kind in kinds:
        mask = (data["kind"] == kind) & np.isclose(data["eta_eV"], reference_eta) & (data["nk"] == reference_nk)
        ax_matsu.plot(data["matsubara_index"][mask], data["reflection_dimensionless_xx"][mask].real, marker="o", label=kind)
    ax_matsu.set_xlabel("Matsubara index n")
    ax_matsu.set_ylabel(r"Re reflection-dimensionless response$_{xx}$")
    ax_matsu.set_title(rf"reflection-normalized response at $N_k={reference_nk}, \eta={reference_eta:g}$")
    style_publication_axis(ax_matsu)
    save_publication_figure(fig_matsu, matsubara_plot)
    plt.close(fig_matsu)
...
def print_summary(data: dict[str, np.ndarray]) -> None:
    max_nk_change = float(np.nanmax(data["relative_change_vs_largest_nk"]))
    max_eta_change = float(np.nanmax(data["relative_change_vs_smallest_eta"]))
    max_symmetry = float(
        max(
    ...
    max_spm_dwave_rel = float(np.nanmax(data["spm_dwave_rel_diff_xx"]))
    statuses = sorted(set(str(item) for item in data["convergence_status"]))
    diagnoses = sorted(set(str(item) for item in data["diagnosis"]))
    print(f"row_count = {data['kind'].size}")
    print(f"max_relative_change_vs_largest_nk = {max_nk_change}")
    print(f"max_relative_change_vs_smallest_eta = {max_eta_change}")
    print(f"max_symmetry_diagnostic = {max_symmetry}")
    print(f"max_spm_dwave_rel_diff_xx = {max_spm_dwave_rel}")
    print(f"convergence_statuses = {statuses}")
    print(f"diagnoses = {diagnoses}")
    print("note = response convergence diagnostic only; not a Casimir result.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-prefix",
        type=Path,
        default=ROOT / "validation" / "outputs" / "archive" / "response" / "convergence_imag" / "data" / "convergence_imag",
    )
    args = parser.parse_args()

    data = convergence_response_imag(
        args.kinds,
        args.nk_list,
        args.eta_list,
        args.matsubara_list,
