...
def output_paths(output_prefix: Path) -> tuple[Path, Path, Path, Path, Path, Path, Path, Path, Path]:
    ...
    project_data_root = (ROOT / "validation" / "outputs" / "archive" / "normal_state" / "fs_adaptive_integration" / "data").resolve()
    if resolved_prefix.is_relative_to(project_data_root):
        figure_dir = ROOT / "validation" / "outputs" / "archive" / "normal_state" / "fs_adaptive_integration" / "figures"
    else:
