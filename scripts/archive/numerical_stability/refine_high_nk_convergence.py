...
def output_paths(output_prefix: Path) -> tuple[Path, Path, Path, Path, Path, Path, Path, Path]:
    ...
    project_data_root = (ROOT / "outputs" / "archive" / "response" / "high_nk_convergence" / "data").resolve()
    if resolved_prefix.is_relative_to(project_data_root):
        figure_dir = ROOT / "outputs" / "archive" / "response" / "high_nk_convergence" / "figures"
    else:
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-prefix",
        type=Path,
        default=ROOT / "outputs" / "archive" / "response" / "high_nk_convergence" / "data" / "high_nk_convergence",
    )
