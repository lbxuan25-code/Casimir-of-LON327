...
def output_paths(output_prefix: Path) -> tuple[Path, Path, Path, Path, Path, Path, Path, Path, Path]:
    ...
    project_data_root = (ROOT / "outputs" / "archive" / "normal_state" / "fs_sensitive_sampling" / "data").resolve()
    if resolved_prefix.is_relative_to(project_data_root):
        figure_dir = ROOT / "outputs" / "archive" / "normal_state" / "fs_sensitive_sampling" / "figures"
    else:
