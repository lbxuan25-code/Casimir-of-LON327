...
def output_paths(output_prefix: Path) -> tuple[Path, Path, Path, Path, Path, Path, Path]:
    ...
    project_root = (ROOT / "outputs" / "casimir" / "local_response_integral" / "convergence" / "data").resolve()
    if resolved_prefix.is_relative_to(project_root):
        figure_dir = ROOT / "outputs" / "casimir" / "local_response_integral" / "convergence" / "figures"
    else:
