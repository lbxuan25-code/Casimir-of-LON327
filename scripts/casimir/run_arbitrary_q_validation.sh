...
out_root="${ARBITRARY_Q_OUT_ROOT:-validation/outputs/matsubara/arbitrary_q_validation/${head_tag}}"
mkdir -p "${out_root}"
...
require_clean() {
  ...
  if [[ -n "${status}" ]]; then
    printf '%s\n' "Formal command requires a clean worktree:" >&2
    printf '%s\n' "${status}" >&2
    exit 2
  fi
}
...
case "${action}" in
  performance-smoke)
    python -m validation diagnostic arbitrary-q-performance-smoke \
      ...
      --output "${out_root}/performance_smoke.json"
    ;;
  ...
  physics-smoke)
    python -m validation diagnostic arbitrary-q-physics-smoke \
      ...
      --output "${out_root}/physics_smoke.json"
    ;;

  diagnostics)
    bash "$0" performance-smoke
    bash "$0" physics-smoke
    ;;
  ...
  performance-preflight)
    ...
    python -m validation matsubara arbitrary-q-performance-preflight \
      ...
      --output "${out_root}/performance_preflight.json"
    ;;

  qualification)
    require_clean
    manifest="${ARBITRARY_Q_PERFORMANCE_MANIFEST:-${out_root}/performance_preflight.json}"
    if [[ ! -f "${manifest}" ]]; then
    ...
    python -m validation matsubara arbitrary-q-periodic-bz-qualification \
      ...
      --ward-tolerance 1e-7 \
      --ward-absolute-tolerance 1e-12 \
      --output "${out_root}/periodic_bz_qualification.json"
    ;;

  help|-h|--help)
    cat <<'EOF'
Usage: bash scripts/casimir/run_arbitrary_q_validation.sh <command>

Commands:
  performance-smoke      Timing breakdown and optimization-structure check
  physics-smoke          Small-N physical closure check without convergence claims
  diagnostics            Run both smoke checks
  performance-preflight  Clean-head real-hardware formal performance preflight
  qualification          Clean-head N=256/384/512 periodic-BZ qualification

Environment:
  ARBITRARY_Q_OUT_ROOT             Override output directory
  ARBITRARY_Q_PERFORMANCE_MANIFEST Override qualification performance manifest

No q-envelope command is provided until the separation range, angle range,
