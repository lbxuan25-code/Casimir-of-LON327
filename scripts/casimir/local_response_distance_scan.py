#!/usr/bin/env python3
"""Compute the preliminary local-response Casimir distance scan."""
...
def full_run_command(
    ...
    *,
    ...
    parts = ["python", "scripts/casimir/local_response_distance_scan.py"]
    option_map = [
        ("--kinds", "kinds"),
...
def _command_path(output_prefix: Path) -> Path:
    if output_prefix.resolve() == DEFAULT_OUTPUT_PREFIX.resolve():
        return COMMAND_PATH
    return output_prefix.parent / "distance_scan_command.sh"


def _write_command_file(command: str, output_prefix: Path) -> Path:
...
def _empty_data() -> dict[str, np.ndarray]:
    return {
        ...
        "preliminary_local_response_conclusion": np.array([], dtype=bool),
            ...
            rows.append(
                {
                    ...
                    "preliminary_local_response_conclusion": _parse_bool(
                        row["preliminary_local_response_conclusion"]
                    ),
...
def save_figures(
    ...
    figure_dir = (
        SCAN_ROOT / "figures"
        if output_prefix.resolve() == DEFAULT_OUTPUT_PREFIX.resolve()
        else output_prefix.parent / "figures"
    )
    ...
    ax.set_title("Local-response Casimir energy")
    ...
    for kind in kinds:
        ...
        for distance in distances:
            mask = (data["kind"] == kind) & np.isclose(data["distance_m"], distance)
            values.append(float(np.nanmax(data["max_abs_torque_over_theta"][mask])) / TORQUE_TOLERANCE)
        ax.plot(distances, values, marker="o", label=kind)
    ax.set_xscale("log")
    ax.set_xlabel("distance (m)")
    ax.set_ylabel(r"$\max|\tau_{\rm fd}|/\tau_{\rm tol}$")
    ax.set_title("Local-response zero-torque baseline")
    ax.axhline(1.0, color="0.25", linestyle="--", linewidth=1.0, label="tolerance")
    style_publication_axis(ax)
    ...
    fig.suptitle("Local-response Casimir energy versus angle", fontsize=10)
    ...
    for ax, kind in zip(axes, kinds, strict=True):
        for distance in distances:
            ...
            ax.plot(
                data["theta"][mask],
                data["torque_fd"][mask] / TORQUE_TOLERANCE,
                marker="o",
                label=f"d={distance:g}",
            )
        ax.axhline(1.0, color="0.35", linestyle="--", linewidth=0.9)
        ax.axhline(-1.0, color="0.35", linestyle="--", linewidth=0.9)
        ax.set_ylabel(r"$\tau_{\rm fd}/\tau_{\rm tol}$")
        ax.set_title(kind)
        style_publication_axis(ax, legend=False)
    axes[-1].set_xlabel("theta (rad)")
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="center left", bbox_to_anchor=(1.0, 0.5), title="distance (m)")
    fig.suptitle("Local-response finite-difference torque versus angle", fontsize=10)
    ...
    if include_toy and np.any(data["kind"] == "toy_anisotropic"):
        ...
        ax.set_title("Anisotropic control")
        style_publication_axis(ax)
...
def _summary_lines(
    ...
    lines.extend(
        [
            ...
            "- benchmark_only=True",
            "- preliminary_local_response_conclusion=True",
            f"preliminary_local_response_conclusion_ready={ready}",
...
def main() -> None:
    ...
    if args.dry_run:
        print(command)
        return
    command_path = _write_command_file(command, args.output_prefix)
    response_cache = (
    ...
    for distance_index, distance in enumerate(args.distance_list, start=1):
        ...
        raw = compute_local_response_casimir_integral(
    ...
    figure_paths = save_figures(data, args.include_toy_anisotropic_control, args.output_prefix)
    ...
    print("note = preliminary local-response Casimir conclusion; not a final Casimir conclusion.")
