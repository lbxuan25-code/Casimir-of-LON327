...
def output_paths(output_prefix: Path) -> tuple[Path, Path, Path, Path, Path]:
    ...
    project_data_root = (ROOT / "outputs" / "archive" / "response" / "static_policy_comparison" / "data").resolve()
    if resolved_prefix.is_relative_to(project_data_root):
        figure_dir = ROOT / "outputs" / "archive" / "response" / "static_policy_comparison" / "figures"
    else:
