...
@pytest.mark.parametrize("vertex_name", ["midpoint", "symmetric_kpm", "bond_endpoint_gauge"])
def test_two_band_collective_vertices_follow_form_factor(pairing_name, vertex_name):
    ...
    expected_amplitude = np.block([[zero, phi], [phi.conjugate().T, zero]]).astype(complex)
    expected_eta2 = np.block([[zero, 1j * phi], [-1j * phi.conjugate().T, zero]]).astype(complex)
    ...
    np.testing.assert_allclose(ansatz.phase_pairing_matrix(kx, ky, qx, qy, amp) / amp.delta0_eV, phi)
...
def test_two_band_dwave_bond_endpoint_form_factor_is_endpoint_average():
    ...
    phi_minus = ansatz.mean_pairing(kx - 0.5 * qx, ky - 0.5 * qy, amp) / delta0
    phi_plus = ansatz.mean_pairing(kx + 0.5 * qx, ky + 0.5 * qy, amp) / delta0

    np.testing.assert_allclose(ansatz.collective_form_factor(kx, ky, qx, qy, amp), 0.5 * (phi_minus + phi_plus))
