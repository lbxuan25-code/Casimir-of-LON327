...
def _artifact(identity) -> CachedCertifiedMaterialResponse:
    ...
    provenance = {
        ...
        for label, shift in zip(labels, identity.shifts)
    }
...
def test_warm_run_never_calls_microscopic_engine(tmp_path: Path, monkeypatch) -> None:
        ...
        return SimpleNamespace(
            frequencies={
                index: SimpleNamespace(
                    xi_eV=cached.matsubara_energy_eV(index, config.temperature_K),
    ...
    cold = cached.evaluate_material_response_ladder_cached(
        config,
        q_crystal=q,
    ...
    warm = cached.evaluate_material_response_ladder_cached(
        config,
        q_crystal=q,
    ...
    assert {row.source for row in warm.frequencies.values()} == {
        "persistent_cache_hit"
    }


def test_changed_certification_sampling_policy_is_a_cache_miss(
    tmp_path: Path,
    monkeypatch,
) -> None:
    _patch_identity_context(monkeypatch)
    config = _config((1,))
    q = np.array([0.015, 0.025])
    identity = cached.build_material_response_cache_identity(
        config,
        q_crystal=q,
        matsubara_index=1,
    ...
    store.put(_artifact(identity))
    changed = replace(config, shifts=((0.5, 0.5), (0.75, 0.25)))
    with pytest.raises(MaterialResponseCacheMiss):
        cached.evaluate_material_response_ladder_cached(
            changed,
            q_crystal=q,
            cache=MaterialResponseCacheStore(tmp_path, mode="read_only"),
        )
...
def test_partial_hit_sends_only_misses_to_microscopic_engine(
    tmp_path: Path,
    monkeypatch,
) -> None:
    _patch_identity_context(monkeypatch)
    ...
    config = _config()
    q = np.array([0.015, 0.025])
    identity_zero = cached.build_material_response_cache_identity(
        config,
        q_crystal=q,
        matsubara_index=0,
        context=cached._identity_context(config),
    ...
    result = cached.evaluate_material_response_ladder_cached(
        config,
        q_crystal=q,
        cache=store,
    )
...
def test_read_only_miss_fails_before_microscopic_fallback(
    tmp_path: Path,
    monkeypatch,
) -> None:
    _patch_identity_context(monkeypatch)
    ...
    with pytest.raises(MaterialResponseCacheMiss):
        cached.evaluate_material_response_ladder_cached(
            _config((1,)),
            q_crystal=np.array([0.015, 0.025]),
            cache=MaterialResponseCacheStore(tmp_path, mode="read_only"),
        )
