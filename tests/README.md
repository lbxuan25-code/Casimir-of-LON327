...
- BdG finite-q q=0 convention helper：`spm` 保持 `convention_aware_pass`，`dwave` 保持 `intraband_aware_pass`；旧 d-wave raw-vs-total mismatch 由 local intraband / `-f'(E)` 解释。
...
- finite-q Ward diagnostic 的 `diagnostic_run_completed=True` 不代表 `ward_identity_closed=True`。
