...
from lno327.finite_q_primitives import (
    ...
    density_vertex as old_density_vertex,
    phase_phase_direct_vertex as old_phase_phase_direct_vertex,
    phase_vertex as old_phase_vertex,
)
...
def test_phase_vertices_match_legacy():
    pairing = np.array(
    ...
    np.testing.assert_allclose(phase_vertex(pairing), old_phase_vertex(pairing))
    np.testing.assert_allclose(
        phase_phase_direct_vertex(delta_theta),
        old_phase_phase_direct_vertex(delta_theta),
    )


def test_density_vertex_matches_legacy_for_four_orbitals():
    np.testing.assert_allclose(density_vertex(4), old_density_vertex())
...
def test_bdg_finite_q_vector_vertex_block_lifting_matches_legacy(direction):
    kx, ky = 0.21, -0.34
    qx, qy = 0.17, -0.09
    ...
    old = old_vector_vertex(kx, ky, qx, qy, direction)

    np.testing.assert_allclose(new, old)


@pytest.mark.parametrize("directions", (("x", "x"), ("y", "y"), ("x", "y")))
def test_bdg_finite_q_contact_vertex_block_lifting_matches_legacy(directions):
    kx, ky = 0.21, -0.34
    qx, qy = 0.17, -0.09
    ...
    new = bdg_finite_q_vertex_from_normal_blocks(particle, hole_normal)
    old = old_contact_vertex(kx, ky, qx, qy, direction_i, direction_j)
