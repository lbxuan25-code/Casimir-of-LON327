...
def test_normal_state_hamiltonian_is_hermitian_and_four_band():
    h = normal_state_hamiltonian(0.37, -0.21)
...
def test_gamma_point_matches_adopted_coefficients_with_mu():
    h = normal_state_hamiltonian(0.0, 0.0)
...
def test_velocity_operator_matches_finite_difference():
    ...
    step = 1e-6
    finite_difference_x = (
        normal_state_hamiltonian(kx + step, ky) - normal_state_hamiltonian(kx - step, ky)
    ) / (2.0 * step)
    finite_difference_y = (
        normal_state_hamiltonian(kx, ky + step) - normal_state_hamiltonian(kx, ky - step)
    ) / (2.0 * step)

    velocity_x = normal_state_velocity_operator(kx, ky, "x")
    velocity_y = normal_state_velocity_operator(kx, ky, "y")
...
def test_c4_related_momenta_have_matching_bands():
    ...
    bands = np.linalg.eigvalsh(normal_state_hamiltonian(kx, ky))
    rotated_bands = np.linalg.eigvalsh(normal_state_hamiltonian(rotated_kx, rotated_ky))
