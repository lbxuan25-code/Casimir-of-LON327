...
from lno327.casimir.production import FullCasimirConfig, build_full_casimir_config, run_full_casimir


def test_package_root_exposes_one_full_calculation_route() -> None:
    ...
    for retired_name in (
        "run_casimir", "FixedCasimirConfig",
        "AdaptiveRadialCasimirConfig", "run_adaptive_radial_casimir",
        "AdaptiveAngularCasimirConfig", "run_adaptive_angular_casimir",
        "AdaptiveJointCasimirConfig", "run_adaptive_joint_casimir",
        "AdaptiveOuterTailCasimirConfig", "run_adaptive_outer_tail_casimir",
        "AdaptiveMatsubaraCasimirConfig", "run_adaptive_matsubara_casimir",
    ):
        assert not hasattr(casimir, retired_name), retired_name


def test_fixed_reference_route_is_explicitly_isolated() -> None:
    ...
    assert metadata["canonical_entrypoint"] == "lno327.casimir.run_full_casimir"
    assert metadata["legacy_fixed_route"] == "lno327.casimir.legacy.run_fixed_reference_casimir"
    assert metadata["legacy_fixed_exported_from_package_root"] is False


def test_canonical_builder_wires_external_physical_inputs() -> None:
    config = build_full_casimir_config(
        pairings=("spm", "dwave"), temperature_K=12.0, separation_nm=25.0,
        plate_angles_deg=(3.0, 29.0), matsubara_cutoff_values=(1, 3, 7, 15),
        cutoff_u_values=(6.0, 12.0, 18.0, 24.0, 30.0, 36.0, 42.0),
        point_cache_path="outputs/casimir/runs/test/cache/certified_points.json",
    )
