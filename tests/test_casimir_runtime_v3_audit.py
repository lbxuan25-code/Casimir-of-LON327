...
def test_transverse_runner_uses_q_file_not_inline_negative_values(
    tmp_path: Path,
    monkeypatch,
) -> None:
    observed = {}
...
def test_scheduling_changes_do_not_invalidate_frequency_cache() -> None:
...
def test_provider_rejects_missing_certifier_rows() -> None:
    def missing(current, manifest, output):
        return _CertificationRun(
            {"schema": "transverse-point-sweet-spot-v4", "point_results": []},
            "",
            "",
            (),
        )
...
def test_current_production_defaults_are_calibrated() -> None:
...
def test_case_state_recognizes_authorized_completed_artifacts(tmp_path: Path) -> None:
    run = tmp_path / "case"
    run.mkdir()
