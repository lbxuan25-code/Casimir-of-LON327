...
from lno327.casimir.material_response import MATERIAL_RESPONSE_IDENTITY_SCHEMA, MaterialResponseSample
...
from lno327.casimir.material_two_plate import TwoPlateGeometryPolicy, assemble_two_plate_logdet
...
def _ward() -> SimpleNamespace:
    ...
    return SimpleNamespace(passed=True, left=side, right=side, schur_condition_number=1.0)
...
def _sample(index: int) -> MaterialResponseSample:
    ...
    if index == 0:
        ...
        sector, strict, basis = "zero_matsubara", SimpleNamespace(passed=True), STATIC_LOCAL_BASIS
    else:
        tensor = ConductivityTensor(0.4, 0.5, 0.02, 0.02)
        sheet = SheetConductivityConversion(tensor=tensor, unit_stage="sheet_conductivity", unit_label="test-sheet", normalization_status="test", valid_for_casimir_input=True, notes=("sheet",))
        tilde = SheetConductivityConversion(tensor=tensor, unit_stage="reflection_dimensionless_conductivity", unit_label="test-tilde", normalization_status="test", valid_for_casimir_input=True, notes=("tilde",))
        response = PositiveMatsubaraSheetResponse(
            sigma_model_xy=tensor, sigma_sheet_si_xy=sheet, sigma_tilde_xy=tilde,
            q_model=q, xi_eV=xi, degeneracy=1.0, basis="crystal_xy",
            metadata={"source": "test"},
        )
        validation = SheetResponseValidation(
            finite=True, relative_imaginary_norm=0.0, relative_symmetry_residual=0.0,
            minimum_symmetric_eigenvalue=0.38, reality_tolerance=1e-9,
            symmetry_tolerance=1e-9, passivity_tolerance=1e-10,
        )
        sector, strict, basis = "positive_matsubara", None, "crystal_xy"
    return MaterialResponseSample(
        frequency_index=0, frequency_sector=sector, q_crystal=q, xi_eV=xi,
        material_cache_fingerprint="grid-fingerprint",
        kernel=SimpleNamespace(q_model=q, xi_eV=xi),
        operator_ward=SimpleNamespace(passed=True), effective_ward=ward,
...
def _cache_identity(index: int, *, q: np.ndarray | None = None) -> MaterialResponseCacheIdentity:
    vector = np.array([0.015, 0.025]) if q is None else np.asarray(q, dtype=float)
    return MaterialResponseCacheIdentity(
        pairing_name="spm", temperature_K=40.0, matsubara_index=index,
        xi_eV=matsubara_energy_eV(index, 40.0), q_crystal=vector,
        ...
        required_consecutive_passes=2, envelope_levels=3,
    )
...
def _artifact(index: int) -> CachedCertifiedMaterialResponse:
    ...
    return CachedCertifiedMaterialResponse(
        identity=identity, snapshot=MaterialResponseSnapshot.from_sample(_sample(index)),
        working_N=64, audit_N=96, primary_shift="shift_0",
        establishment_mode="strict_consecutive_adjacent",
        certification_evidence={"convergence_policy": dict(identity.convergence_policy), "valid_for_casimir_input": False, "production_casimir_allowed": False},
...
def _conflicting_artifact() -> CachedCertifiedMaterialResponse:
    ...
    response = PositiveMatsubaraSheetResponse(
        sigma_model_xy=tensor, sigma_sheet_si_xy=sheet, sigma_tilde_xy=tilde,
        q_model=artifact.snapshot.q_crystal, xi_eV=artifact.snapshot.xi_eV,
        degeneracy=1.0, basis="crystal_xy", metadata={"source": "conflict-test"},
    )
    validation = SheetResponseValidation(
        finite=True, relative_imaginary_norm=0.0, relative_symmetry_residual=0.0,
        minimum_symmetric_eigenvalue=0.78, reality_tolerance=1e-9,
        symmetry_tolerance=1e-9, passivity_tolerance=1e-10,
    )
    snapshot = replace(artifact.snapshot, response=response, sheet_validation=validation)
    return replace(artifact, snapshot=snapshot)
...
def test_identity_is_exact_and_geometry_free() -> None:
    ...
    shifted = _cache_identity(1, q=np.array([0.015, np.nextafter(0.025, 1.0)]))
    assert shifted.sha256 != identity.sha256
    restored = MaterialResponseCacheIdentity.from_payload(identity.payload)
    assert restored.payload == identity.payload
...
def test_cache_roundtrip_is_readonly_and_geometry_equivalent(tmp_path: Path, index: int) -> None:
    ...
    np.testing.assert_array_equal(loaded.snapshot.primary_matrix, artifact.snapshot.primary_matrix)
    q = artifact.snapshot.q_crystal
    policy = TwoPlateGeometryPolicy(separation_m=100e-9)
    live = assemble_two_plate_logdet(_sample(index), _sample(index), q_lab=q, theta_1_rad=0.0, theta_2_rad=0.0, policy=policy)
    replay = assemble_two_plate_logdet(loaded.snapshot, loaded.snapshot, q_lab=q, theta_1_rad=0.0, theta_2_rad=0.0, policy=policy)
...
def test_write_failure_cannot_leave_loader_visible_final_file(tmp_path: Path, monkeypatch) -> None:
    store = MaterialResponseCacheStore(tmp_path, mode="populate")
    artifact = _artifact(1)
    final_path = store.path_for(artifact.identity)
    monkeypatch.setattr(cache_store_module, "_write_npz", lambda *args, **kwargs: (_ for _ in ()).throw(OSError("simulated write failure")))
...
def test_same_identity_conflict_never_overwrites_existing_artifact(tmp_path: Path) -> None:
    ...
    np.testing.assert_array_equal(reloaded.snapshot.primary_matrix, stored.snapshot.primary_matrix)
