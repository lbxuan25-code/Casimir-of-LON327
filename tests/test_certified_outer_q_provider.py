from __future__ import annotations

from pathlib import Path

import numpy as np
import pytest

from lno327.casimir.certified_point_provider import (
    CertifiedOuterQProvider,
    CertifiedPointCacheError,
)
from lno327.casimir.fixed_chain import FixedCasimirConfig, _CertificationRun


def _point(pairing: str, label: str, n: int, value: float) -> dict:
    return {
        "pairing": pairing,
        "q_label": label,
        "n": n,
        "sweet_spot": {
            "status": "established",
            "working_N": 192,
            "audit_N": 256,
            "establishment_mode": "strict_consecutive_adjacent",
        },
        "history": [
            {
                "N": 256,
                "two_plate_logdet_cross_shift": {"passed": True},
                "shifts": {
                    "primary": {
                        "two_plate_logdet": value,
                        "hard_physical_passed": True,
                    }
                },
            }
        ],
    }


def test_incremental_provider_reuses_exact_q_points_and_persists_cache(
    tmp_path: Path,
) -> None:
    config = FixedCasimirConfig()
    calls = []

    def runner(current, manifest, output):
        calls.append(tuple(manifest.labels))
        payload = {
            "schema": "transverse-point-sweet-spot-v4",
            "run_complete": True,
            "all_requested_sweet_spots_established": True,
            "point_results": [
                _point(pairing, label, n, float(index + n + 1))
                for index, label in enumerate(manifest.labels)
                for pairing in current.pairings
                for n in current.matsubara_indices
            ],
        }
        return _CertificationRun(payload, "", "", ("python",))

    cache = tmp_path / "points.json"
    q = np.asarray([[0.1, 0.2], [0.3, 0.4]], dtype=float)
    provider = CertifiedOuterQProvider(config, cache_path=cache, runner=runner)
    first = provider.evaluate(q)
    second = provider.evaluate(q[::-1])

    assert first.all_established
    assert first.new_q_count == 2
    assert second.new_q_count == 0
    assert second.cache_hit_q_count == 2
    assert len(calls) == 1
    assert provider.unique_q_count == 2
    assert provider.cached_point_count == 4
    assert cache.exists()

    def forbidden_runner(*args, **kwargs):
        raise AssertionError("persisted exact points must not be recomputed")

    restored = CertifiedOuterQProvider(
        config,
        cache_path=cache,
        runner=forbidden_runner,
    )
    replay = restored.evaluate(q)
    assert replay.all_established
    assert replay.new_q_count == 0
    assert restored.primary_logdet("spm", 0, q[0]) == 1.0


def test_incremental_provider_rejects_cache_from_a_different_point_policy(
    tmp_path: Path,
) -> None:
    cache = tmp_path / "points.json"
    base = FixedCasimirConfig()

    def runner(current, manifest, output):
        payload = {
            "schema": "transverse-point-sweet-spot-v4",
            "point_results": [
                _point(pairing, label, n, 1.0)
                for label in manifest.labels
                for pairing in current.pairings
                for n in current.matsubara_indices
            ],
        }
        return _CertificationRun(payload, "", "", ("python",))

    CertifiedOuterQProvider(base, cache_path=cache, runner=runner).evaluate(
        np.asarray([[0.1, 0.2]])
    )
    changed = FixedCasimirConfig(plate_angles_deg=(0.0, 19.0))
    with pytest.raises(CertifiedPointCacheError, match="fingerprint"):
        CertifiedOuterQProvider(changed, cache_path=cache, runner=runner)
