...
from lno327.numerics.weights import k_weights as normalized_k_weights
from lno327.response.config import KuboConfig
from lno327.response.local_bdg import bdg_local_total_kernel_imag_axis
from lno327.response.local_interface import LocalSheetResponse, ResponseKind, local_response_imag_axis
...
def local_response_matsubara_index(
    ...
    components = bdg_local_total_kernel_imag_axis(
        spec,
        kind,
        points,
        config,
    ...
    return StaticResponseResult(
        ...
        source="bdg_local_total_kernel_imag_axis_at_zero_frequency",
        valid_for_casimir_input=False,
        notes=(
...
def matsubara_response_series(
    ...
    eta_eV: float = 1e-4,
    pairing_params: PairingAmplitudes | None = None,
) -> list[StaticResponseResult]:
    indices = np.asarray(n_values, dtype=int)
    if indices.ndim != 1 or indices.size == 0:
        raise ValueError("n_values must be a non-empty 1D array")
    return [
