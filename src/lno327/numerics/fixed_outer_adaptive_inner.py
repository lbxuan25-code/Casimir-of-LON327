"""Fixed global Gauss outer rule with shared vector-adaptive inner integration.

This module owns the retained numerical definition used by the d-wave static
reference.  The outer coordinate is integrated by one deterministic global
Gauss-Legendre rule.  At each outer node the orthogonal coordinate is integrated
by one vector-valued ``scipy.integrate.quad_vec`` call, so every primitive and
Ward-RHS channel shares the same inner nodes and weights.

The reported error contains weighted inner errors only.  Outer discretization is
an independent error source and must be assessed with an outer-order ladder and
an ``xy``/``yx`` orientation audit.
"""
...
class EvaluationBudgetExceeded(RuntimeError):
    """Raised when one orientation exceeds its microscopic point budget."""

    def __init__(self, maximum: int, attempted: int):
        super().__init__(
            "fixed-outer/adaptive-inner integration exceeded max_point_evaluations: "
...
class FixedOuterAdaptiveInnerOptions:
    """Frozen numerical controls for the adaptive inner integral."""
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("inner_limit", "max_point_evaluations", "cache_size_bytes"):
...
@dataclass(frozen=True)
class FixedOuterAdaptiveInnerOrientationResult:
    """One orientation result and its inner-adaptive diagnostics."""
    ...
    def __post_init__(self) -> None:
        value = np.array(self.value, dtype=float, copy=True)
        if value.ndim != 1 or not np.isfinite(value).all():
            raise ValueError("orientation result must be a finite one-dimensional vector")
...
def integrate_fixed_outer_adaptive_inner_orientation(
    ...
    *,
    ...
    """Integrate one orientation of a finite real vector-valued BZ integrand."""
    ...
    def microscopic_value(outer_value: float, inner_value: float) -> np.ndarray:
        ...
        if value.ndim != 1 or not np.isfinite(value).all():
            raise ValueError(
                "fixed-outer/adaptive-inner integrand must return a finite "
        ...
        if expected_shape is None:
            ...
            raise ValueError(
                "fixed-outer/adaptive-inner integrand changed vector shape: "
    ...
    return FixedOuterAdaptiveInnerOrientationResult(
