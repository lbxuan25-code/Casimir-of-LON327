...
These helpers prepare dimensionless sheet conductivity for later electromagnetic
boundary-condition audits.  They do not compute Lifshitz trace logs, Casimir
...
from .constants import C0, EV_TO_J, HBAR
...
def model_q_to_si_wavevector(
    ...
    """Convert dimensionless model momentum to SI in-plane wave vector."""
...
def omega_eV_to_xi_si(omega_eV: float) -> float:
    """Convert Matsubara energy hbar*xi in eV to xi in s^-1."""
...
def rotate_sigma_tilde_xy_to_lt(
    ...
    *,
    ...
    """Return sigma_tilde_LT = R_Q sigma_tilde_xy R_Q^T."""
...
def vacuum_admittance_LT(xi_si: float, kappa_m_inv: float) -> np.ndarray:
    """Return Y0 = diag(xi/(c*kappa), c*kappa/xi) in LT order."""
...
def tangential_electric_reflection_matrix_LT(
    ...
    """Return R_E^LT = - solve(2*Y0 + sigma_tilde_LT, sigma_tilde_LT)."""
...
def reflection_input_metadata(*, q_zero_basis_convention: str | None = None) -> dict[str, Any]:
    """Return metadata for Stage 5.5b reflection-input formatting."""
