...
logdet. This module supplies the replacement numerical contract: compare static
susceptibility channels or positive-Matsubara crystal-frame sheet tensors before
any plate rotation, reflection, separation, or outer quadrature is introduced.
"""
...
class MaterialResponseConvergencePolicy:
    """Provisional response-space convergence policy for TODO 2 qualification."""
...
def _absolute_then_relative(
    ...
    *,
    ...
    if (
        not np.isfinite(absolute_value)
        or absolute_value < 0.0
...
class MaterialResponseLevelAssessment:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            MappingProxyType(
                {key: MappingProxyType(dict(value)) for key, value in self.cross_shift_comparisons.items()}
            ),
        )
        adjacent = self.adjacent_N_by_shift
        if adjacent is not None:
            adjacent = MappingProxyType(
                {key: MappingProxyType(dict(value)) for key, value in adjacent.items()}
            )
        object.__setattr__(self, "adjacent_N_by_shift", adjacent)
        object.__setattr__(self, "cross_shift_all_passed", bool(self.cross_shift_all_passed))
        object.__setattr__(
            self,
...
def assess_material_response_level(
    *,
    ...
    if len(set(labels)) != len(labels):
        raise ValueError("shift labels must be unique")
    ...
    cross_shift = {
        f"{left_label}|{right_label}": compare_material_responses(
            current[left_label],
            current[right_label],
            policy=policy,
        )
    ...
    adjacent: dict[str, Mapping[str, Any]] | None = None
...
def assess_material_response_envelope(
    ...
    *,
    ...
    """Assess a joint multi-N, multi-shift oscillatory response envelope."""
    ...
    anchor = window[0].samples_by_shift[next(iter(window[0].samples_by_shift))]
    comparisons: dict[str, Any] = {}
    for row in window:
        for shift_label, sample in row.samples_by_shift.items():
            key = f"N{row.n_grid}:{shift_label}"
            comparisons[key] = compare_material_responses(
                anchor,
                sample,
                policy=policy,
            )
    ...
    return {
        "available": True,
        "levels": count,
        "N_window": [row.n_grid for row in window],
...
class CertifiedMaterialResponse:
    ...
    def __post_init__(self) -> None:
        ...
        primary_shift = str(self.primary_shift)
        if primary_shift not in samples:
            raise ValueError("primary_shift is absent from audit_samples_by_shift")
        reference = samples[primary_shift]
        ...
        object.__setattr__(self, "primary_shift", primary_shift)
        object.__setattr__(
            self,
...
def certify_material_response_history(
    ...
    *,
    ...
    working = history[-2]
    audit = history[-1]
    primary_shift = next(iter(audit.samples_by_shift))
    mode = (
